...
double AGM::GreenfunctionReactionDiffusion::green_function_t(double t) const {
    if (t < tau || isclose(tm, t)) {
        ...
        return -std::pow(alpha, 2) / 2.0E0 / c * E(tau, tm) / E(tp, tm) *
               (std::exp(-alpha * (t - tau)) + std::exp(-alpha * (2.0E0 * tp - t - tau)));
    }
}
...
double AGM::GreenfunctionReactionDiffusion::green_function_tau(double t) const {
    if (t < tau || isclose(tm, t)) {
        ...
        return std::pow(alpha, 2) / 2.0E0 / c * E(tp, t) / E(tp, tm) *
               (std::exp(-alpha * (t - tau)) + std::exp(-alpha * (t + tau - 2.0E0 * tm)));
    }
}
...
double AGM::GreenfunctionReactionDiffusion::green_function_ttau(double t) const {
    if (t < tau || isclose(tm, t)) {
        return -std::pow(alpha, 3) / 2.0E0 / c / E(tp, tm) * (E(t, tm) * E(tp, tau) * std::exp(-alpha * (tau - t)) +
                                                              ...
                                                              2.0E0 * std::exp(-alpha * (t + tau - 2.0E0 * tm)));
    } else {
        return -std::pow(alpha, 3) / 2.0E0 / c / E(tp, tm) * (E(tau, tm) * E(tp, t) * std::exp(-alpha * (t - tau)) +
                                                              2.0E0 * std::exp(-alpha * (2.0E0 * tp - t - tau)) +
                                                              2.0E0 * std::exp(-alpha * (t + tau - 2.0E0 * tm)));
    }
}
...
double AGM::GreenfunctionReactionDiffusion::green_function_t_ND(double t) const {
    if (t < tau || isclose(tm, t)) {
        ...
        return -std::pow(alpha, 2) / 2.0E0 / c * F(tau, tm) / F(tp, tm) *
               (std::exp(-alpha * (t - tau)) + std::exp(-alpha * (2.0E0 * tp - t - tau)));
    }
}
...
double AGM::GreenfunctionReactionDiffusion::green_function_tau_ND(double t) const {
    if (t < tau || isclose(tm, t)) {
        ...
        return std::pow(alpha, 2) / 2.0E0 / c * E(tp, t) / F(tp, tm) *
               (std::exp(-alpha * (t - tau)) - std::exp(-alpha * (t + tau - 2.0E0 * tm)));
    }
...
double AGM::GreenfunctionReactionDiffusion::green_function_ttau_ND(double t) const {
    if (t < tau || isclose(tm, t)) {
        ...
        return -std::pow(alpha, 3) / 2.0E0 / c / E(tp, tm) * (F(tau, tm) * E(tp, t) * std::exp(-alpha * (t - tau)) +
                                                              2.0E0 * std::exp(-alpha * (2.0E0 * tp - t - tau)) -
                                                              2.0E0 * std::exp(-alpha * (t + tau - 2.0E0 * tm)));
    }
}
...
double AGM::GreenfunctionReactionDiffusion::green_function_t_DN(double t) const {
    if (t < tau || isclose(tm, t)) {
        ...
        return -std::pow(alpha, 2) / 2.0E0 / c * E(tau, tm) / F(tp, tm) *
               (std::exp(-alpha * (t - tau)) - std::exp(-alpha * (2.0E0 * tp - t - tau)));
    }
}
...
double AGM::GreenfunctionReactionDiffusion::green_function_tau_DN(double t) const {
    if (t < tau || isclose(tm, t)) {
        ...
        return std::pow(alpha, 2) / 2.0E0 / c * F(tp, t) / F(tp, tm) *
               (std::exp(-alpha * (t - tau)) + std::exp(-alpha * (t + tau - 2.0E0 * tm)));
    }
...
double AGM::GreenfunctionReactionDiffusion::green_function_ttau_DN(double t) const {
    if (t < tau || isclose(tm, t)) {
        ...
        return -std::pow(alpha, 3) / 2.0E0 / c / F(tp, tm) * (E(tau, tm) * F(tp, t) * std::exp(-alpha * (t - tau)) -
                                                              2.0E0 * std::exp(-alpha * (2.0E0 * tp - t - tau)) +
                                                              2.0E0 * std::exp(-alpha * (t + tau - 2.0E0 * tm)));
    }
}
    ...
    switch (i) {
        case 'l':
            value = l1m(tau) - (l1m(tm) + l1p(tm)) * std::exp(-alpha * (tau - tm)) +
                    l1p(tau) * std::exp(-2.0E0 * alpha * (tau - tm));
            return alpha / 2.0E0 / c * E(tp, tau) / E(tp, tm) * value;
        case 'r':
            value = l1p(tau) - (l1p(tp) + l1m(tp)) * std::exp(-alpha * (tp - tau)) +
                    l1m(tau) * std::exp(-2.0E0 * alpha * (tp - tau));
            return alpha / 2.0E0 / c * E(tau, tm) / E(tp, tm) * value;
        default:
            return ZEROVALUE;
    }
