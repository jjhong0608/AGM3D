...
//                                  wvel.at(i), pts->at(i));
    ...
    auto updateTime = [&]() -> void {
        ...
        if (isclose(std::floor(pointHeat::getTime() + HALFVALUE * pointHeat::getDelta()), pointHeat::getTime())) {
            wf.writeResult("/home/jjhong0608/docker/AGM3D/Navier-Stokes/Lid-driven_cavity/Re1000_2/AGM_Result_" + std::to_string(pointHeat::getTime()));
        }
    };
    copyPointInformation();
    assignInitial();
    ...
    wf.writeResult("/home/jjhong0608/docker/AGM3D/Navier-Stokes/Lid-driven_cavity/Re1000_2/AGM_Result");
}
